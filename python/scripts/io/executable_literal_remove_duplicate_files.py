...
Results = namedtuple("Results", ["remove", "keep"])
...
def determine_originals(file_paths: list[str], num_keep: int) -> Results:
    ...
    -----------
        ...
        - `num_keep: The number of duplicates to keep
    """
    ...
    for i, path in enumerate(oldest_to_newest):
        if i < num_keep:
            ...
            remove.append(path)
    return Results(remove=remove, keep=keep)
